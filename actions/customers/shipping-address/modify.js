// Shipping address modification action
import { putRequest } from '../../../src/utils/neviosExpress';

/**
 * Updates a shipping address for a customer
 * 
 * @param {string} customerId - The ID of the customer
 * @param {string} addressId - The ID of the shipping address to update
 ...
 */
export async function modifyShippingAddress(customerId, addressId, updateData, setAsDefault = false) {
  try {
    if (!customerId) {
    ...
    if (updateData.point_id !== undefined) payload.point_id = updateData.point_id;
    if (updateData.point_name !== undefined) payload.point_name = updateData.point_name;
    if (updateData.point_carrier !== undefined) payload.point_carrier = updateData.point_carrier;
    if (updateData.store !== undefined) payload.store = updateData.store;
    
    // Build URL with query parameter if setAsDefault is true
    let endpoint = `/server/customers/shipping-address/${customerId}/${addressId}`;
    if (setAsDefault) {
