...
/**
 * NeviosCircuit - Decorative animated circuit board component
 * 
 * A visually striking circuit board animation with pulsing effects.
 * Features clickable interaction to restart the animation.
 * 
 * @param {Object} props - Component props
 * @param {number} [props.size=110] - Size of the circuit board (width and height in pixels)
 * @param {number} [props.borderRadius=1.5] - Border radius multiplier for rounded corners
 * @param {string} [props.outlineColor='#3b82f6'] - Color for the animated pulsing outline border (hex, rgb, or CSS color)
 * 
 * @example
 * ```jsx
 * import { NeviosCircuit } from './nevios/NeviosCircuit';
 * 
 ...
 * ```
 */
export function NeviosCircuit({ 
  size = 110, 
  borderRadius = 1.5, 
  outlineColor = '#3b82f6' // Default blue-500
}) {
  return <CircuitBoard size={size} borderRadius={borderRadius} outlineColor={outlineColor} />;
}
...
// Outline pulsing animation - creates a flashing/dimming effect
const outlinePulse = (color) => keyframes`
  0%, 100% {
    border-color: ${color};
    opacity: 1;
...
function CircuitBoard({ size, borderRadius: roundedness, outlineColor }) {
  const theme = useTheme();
  const [key, setKey] = React.useState(0);
  
  // Calculate scale factor based on default size of 110px
  const scale = size / 110;
  
  // Create the pulsing animation for the outline
  const outlinePulseAnimation = React.useMemo(() => outlinePulse(outlineColor), [outlineColor]);
        ...
        >
          <Box
            ...
            sx={{
              ...
              animation: `${turnOn} 1.25s ease-in, ${outlinePulseAnimation} 2s ease-in-out infinite`,
          ...
          >
            ...
            <Box
              sx={{
                ...
                boxShadow: `inset 0 0px ${12 * scale}px 0 hsla(0,0%,0%,0.5)`,
                zIndex: 2,
              }}
            >
              <Box
              ...
              <SmallCircuitConnectors top={27 * scale} left={9 * scale} scale={scale}/>
              <SmallCircuitConnectors top={47 * scale} left={9 * scale} scale={scale}/>
              <SmallCircuitConnectors top={27 * scale} left={65 * scale} scale={scale}/>
              <SmallCircuitConnectors top={47 * scale} left={65 * scale} scale={scale}/>
              <Box
                sx={{
                  position: 'absolute',
                  inset: 12 * scale,
                  ...
                  backdropFilter: `blur(${1 * scale}px)`,
                }}
              >
                <Box
                  sx={{
...
function SmallCircuitConnectors({ top = 27, left = 5, scale = 1 }) {
  return (
    <Box
      sx={{
        position: 'absolute',
...
function SmallCircuits({ scale = 1 }) {
  return (
    <Box
      sx={{
        opacity: 0.75,
